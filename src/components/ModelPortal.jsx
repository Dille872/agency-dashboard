...
// v3.40.0: Prioritäts-Styles für "Meine Aufgaben"
const TODO_PRIORITY = {
...
export default function ModelPortal({ session, displayName: initialDisplayName, onSwitchToAdmin, isPreview, unreadCustomContent, onMarkCustomContentRead }) {
  ...
  const loadContentRequests = async () => {
    // Models sehen nur bestätigte und erledigte Aufträge — keine 'angefragt' (noch nicht freigegeben),
    // keine 'abgelehnt', keine 'neu' (intern)
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-base)', fontFamily: 'var(--font-sans)', color: 'var(--text-primary)' }}>
      ...
      <main style={{ padding: '20px', maxWidth: 1100, margin: '0 auto' }}>
                    ...
                    {isExpanded && (
                      <div style={{ padding: '10px 12px 12px', borderTop: `1px solid ${borderColor}22` }}>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 10 }}>
                          {isPreview && <div><div style={{ fontSize: 10, color: 'var(--text-muted)', marginBottom: 2 }}>Angefragt von</div><div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-primary)' }}>{cc.requested_by || '—'}</div></div>}
        ...
        {activeSection === 'anfragen' && (() => {
          const activeReqs = contentRequests.filter(r => r.status === 'bestaetigt')
          const doneReqs = contentRequests.filter(r => r.status === 'erledigt')
          const renderReq = (req) => {
            const statusColor = req.status === 'erledigt' ? '#10b981' : req.status === 'bestaetigt' ? '#06b6d4' : '#f59e0b'
            const statusLabel = req.status === 'erledigt' ? '✓ Erledigt' : req.status === 'bestaetigt' ? '✓ Bestätigt' : ''
            const remainder = (req.price || 0) - (req.deposit || 0)
