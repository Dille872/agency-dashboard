...
export default function TodoTab({ session, userDisplayName }) {
  ...
  const loadAdmins = async () => {
    const { data } = await supabase.from('user_roles').select('display_name').in('role', ['admin', 'manager'])
    setAdminNames([...new Set((data || []).map(a => a.display_name))])
  }
          ...
          {openTodos.map(todo => {
            ...
            return (
              <div key={todo.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 12px', borderRadius: 8, marginBottom: 6, background: color + '06', border: `0.5px solid ${color}33` }}>
                <div onClick={() => toggleTodo(todo)} style={{ width: 18, height: 18, borderRadius: 4, flexShrink: 0, marginTop: 2, cursor: 'pointer', border: `1.5px solid ${color}`, background: 'transparent' }} />
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, marginTop: 5, alignItems: 'center', flexWrap: 'wrap' }}>
                    ...
                    {todo.assigned_to && (
                      <span style={{ fontSize: 10, fontWeight: 600, padding: '1px 7px', borderRadius: 4, background: 'rgba(124,58,237,0.15)', color: '#a78bfa' }}>→ {todo.assigned_to}</span>
                    )}
        ...
        <div style={cardS}>
          ...
          {doneTodos.map(todo => (
            <div key={todo.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 12px', borderRadius: 8, marginBottom: 6, background: 'var(--bg-card2)', border: '0.5px solid var(--border)', opacity: 0.55 }}>
              <div onClick={() => toggleTodo(todo)} style={{ width: 18, height: 18, borderRadius: 4, flexShrink: 0, marginTop: 2, cursor: 'pointer', background: '#10b981', border: '1.5px solid #10b981', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                {todo.assigned_to && <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 2 }}>war für {todo.assigned_to}</div>}
                <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 2 }}>erledigt von {todo.completed_by} · {formatDate(todo.completed_at)}</div>
              </div>
              <button onClick={() => deleteTodo(todo.id)} style={{ background: 'transparent', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontSize: 13, padding: '0 4px', flexShrink: 0 }}
                onMouseEnter={e => e.target.style.color = '#ef4444'} onMouseLeave={e => e.target.style.color = 'var(--text-muted)'}>✕</button>
            </div>
